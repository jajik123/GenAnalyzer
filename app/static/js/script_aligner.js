...
async function submitSequences() {
    ...
    try {
        ...
        if (result.error) {
            ...
            drawD3Alignment(aligned_sequence1, aligned_sequence2);
...
function drawD3Alignment(reference, input) {
    const svg = d3.select("#alignmentD3");
...
function createChangesTable(changes) {
    ...
    // Check if the table exists, and if so, clear it; otherwise, create a new table
